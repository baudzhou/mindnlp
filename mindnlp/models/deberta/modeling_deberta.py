...
class DebertaLayerNorm(nn.Cell):
    ...
    def __init__(self, size, eps=1e-12):
        super().__init__()
        self.weight = ms.Parameter(ops.ones(size))
        self.bias = ms.Parameter(ops.zeros(size))
...
class DisentangledSelfAttention(nn.Cell):
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if "c2p" in self.pos_att_type:
            ...
            pos_key_layer = pos_key_layer.transpose(0, 1, -1, -2)
            c2p_att = ops.matmul(query_layer, pos_key_layer)
            c2p_pos = ops.clip(relative_pos + att_span, 0, att_span * 2 - 1)
            c2p_pos = c2p_dynamic_expand(c2p_pos, query_layer, relative_pos)
            c2p_att = self.gather(c2p_att, -1, c2p_pos)
        ...
        if "p2c" in self.pos_att_type:
            ...
            pos_query_layer = pos_query_layer.transpose(0, 1, -1, -2)#.to(dtype=key_layer.dtype)
            p2c_att = ops.matmul(key_layer, pos_query_layer)
            p2c_pos = p2c_dynamic_expand(p2c_pos, query_layer, key_layer)
            p2c_att = self.gather(
                p2c_att, -1, p2c_pos
            ).transpose(0, 1, -1, -2)
            ...
            if query_layer.shape[-2] != key_layer.shape[-2]:
                ...
                p2c_att = self.gather(p2c_att, -2, pos_dynamic_expand(pos_index, p2c_att, key_layer))
            score += p2c_att
