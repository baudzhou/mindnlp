...
class XSoftmax(torch.autograd.Function):
    ...
    def construct(self, input: Tensor, mask: Tensor, dim):
        self.dim = dim
        rmask = ~(mask.to(ms.bool))

        output = input.masked_fill(rmask, Tensor(np.finfo(input.dtype).min))
...
def get_mask(input, local_context):
    ...
    if dropout > 0 and mask is None:
        ...
        mask = (1 - ms.numpy.empty_like(input).bernoulli(1 - dropout)).to(ms.bool)
...
class StableDropout(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.training and self.drop_prob > 0:
            return self.dropout(x, self.get_context())
        return x
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(
                    create_custom_construct(layer_module),
                    next_kv,
                    attention_mask,
                    query_states,
                    relative_pos,
...
def build_relative_position(query_size, key_size, device):
    ...
    q_ids = ops.arange(0, query_size, dtype=ms.long, device=device)
    k_ids = ops.arange(0, key_size, dtype=ms.long, device=device)
...
class DisentangledSelfAttention(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.q_bias = ms.Parameter(ops.zeros((self.all_head_size), dtype=ms.float))
        self.v_bias = ms.Parameter(ops.zeros((self.all_head_size), dtype=ms.float))
    ...
    def construct(
        ...
        scale = ops.sqrt(Tensor(query_layer.shape[-1], dtype=ms.float) * scale_factor)
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if "c2p" in self.pos_att_type:
            ...
            c2p_att = ops.matmul(query_layer, pos_key_layer.transpose(-1, -2))
            c2p_pos = ops.clip(relative_pos + att_span, 0, att_span * 2 - 1)
            c2p_att = ops.gather(c2p_att, dim=-1, input_indices=c2p_dynamic_expand(c2p_pos, query_layer, relative_pos))
            score += c2p_att
        ...
        if "p2c" in self.pos_att_type:
            ...
            pos_query_layer /= ops.sqrt(Tensor(pos_query_layer.shape[-1], dtype=ms.float) * scale_factor)
            ...
            p2c_att = ops.matmul(key_layer, pos_query_layer.transpose(-1, -2).to(dtype=key_layer.dtype))
            p2c_att = ops.gather(
                p2c_att, axis=-1, input_indices=p2c_dynamic_expand(p2c_pos, query_layer, key_layer)
            ).transpose(-1, -2)

            if query_layer.shape[-2] != key_layer.shape[-2]:
                pos_index = relative_pos[:, :, :, 0].unsqueeze(-1)
                p2c_att = ops.gather(p2c_att, axis=-2, input_indices=pos_dynamic_expand(pos_index, p2c_att, key_layer))
            score += p2c_att
...
class DebertaEmbeddings(nn.Cell):
    ...
    def construct(self, input_ids=None, token_type_ids=None, position_ids=None, mask=None, inputs_embeds=None):
        ...
        if token_type_ids is None:
            token_type_ids = ops.zeros(input_shape, dtype=torch.long, device=self.position_ids.device)
...
class DebertaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Dense):
            ...
            module.weight = ops.normal(module.weight.shape, mean=0, std=self.config.initializer_range, dtype=module.weight.dtype)
            ...
            if module.padding_idx is not None:
                ...
                module.weight[module.padding_idx] = ops.zeros_like(module.weight[module.padding_idx])
...
class DebertaModel(DebertaPreTrainedModel):
    ...
    def construct(
        ...
        if attention_mask is None:
            attention_mask = ops.ones(input_shape, device=device)
        if token_type_ids is None:
            token_type_ids = ops.zeros(input_shape, dtype=ms.long, device=device)
